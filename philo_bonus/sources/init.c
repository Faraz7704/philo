...
/*   Updated: 2022/09/27 19:14:25 by fkhan            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	init_thsize_t(t_pinfo *pinfo)
{
	if (init_forks(pinfo, pinfo->amount))
		return (1);
	if (thsize_t_init(&pinfo->finish, "finish", pinfo->amount))
		return (1);
	if (thsize_t_init(&pinfo->write, "write", 0))
		return (1);
	if (thsize_t_init(&pinfo->quit, "quit", 0))
		return (1);
	if (thsize_t_init(&pinfo->stop_process, "stop_process", 0))
		return (1);
	return (0);
}
